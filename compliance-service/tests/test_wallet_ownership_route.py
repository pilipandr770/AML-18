...
def test_requirement_check_below_threshold(client):
...
def test_requirement_check_at_and_above_threshold(client):
    resp = client.get("/wallet-ownership/requirement?transfer_amount_eur=1000")
    assert resp.get_json()["required"] is True

    resp = client.get("/wallet-ownership/requirement?transfer_amount_eur=5000")
    assert resp.get_json()["required"] is True


def test_requirement_check_rejects_non_numeric(client):
    resp = client.get("/wallet-ownership/requirement?transfer_amount_eur=not-a-number")
    assert resp.status_code == 400


def test_signed_message_flow_verifies_correct_signer(client, app):
...
def test_signed_message_flow_rejects_wrong_signer(client):
...
def test_signed_message_flow_rejects_replayed_challenge(client):
    account = Account.create()
...
def test_signed_message_flow_rejects_unknown_challenge(client):
...
def test_verification_rejects_unknown_method(client):
    resp = client.post("/wallet-ownership/verifications", json={
...
def test_test_transfer_requires_configuration(client):
    resp = client.post("/wallet-ownership/verifications", json={
...
def test_test_transfer_flow_reaches_verified(client, app, monkeypatch):
    ...
    first_poll = client.get(f"/wallet-ownership/verifications/{started['verification_id']}")
    assert first_poll.get_json()["status"] == "pending"

    second_poll = client.get(f"/wallet-ownership/verifications/{started['verification_id']}")
