...
## 2026-09-12 Update: Open-core hosted SaaS tier (multi-tenancy + Stripe billing)

Turned the project into an open-core product on request: the self-hosted
version stays free and completely unchanged, and a centrally-hosted,
paid SaaS tier can now run from the exact same codebase, switched on
purely via `BILLING_ENABLED` (default `false`).

Scope, deliberately narrowed and confirmed with the user before building:
only `wallet_ownership` and `ageverify` go multi-tenant/hosted -- both
were already gated per-call by `require_api_key()`, so the tenant
boundary already existed, just never used for data isolation. Travel-
Rule/TRISA screening stays strictly self-host-only: each TRISA node is
its own VASP identity with its own certs and GDS registration, so
multi-tenanting it would mean hosting a separate TRISA identity per
customer -- a materially bigger, separate project, out of scope here.

- **Closed a real cross-tenant data leak.** Before this change, any valid
  API key could read or act on *any* project's wallet-ownership/age-
  verify rows by ID -- none of `WalletOwnershipChallenge`,
  `WalletOwnershipVerification`, `AgeVerificationSession`,
  `AgeVerification` had an owner column, and no route filtered by the
  authenticated project. Added `developer_project_id` (FK, NOT NULL) to
  all four, threaded it through every service function and route, and
  proved the fix with a dedicated `test_tenant_isolation.py` plus a live
  check against the real Postgres-backed stack (two freshly registered
  projects; project B got `404`/"unknown challenge" on every attempt to
  touch project A's data).
- **`app/billing/`**: a `UsageEvent` append-only log (same audit-of-record
  shape as `AuditLog`/`ListSnapshot`), `metering.py` (record + calendar-
  month quota check), a thin `stripe_client.py` wrapper (Checkout
  Sessions, webhook signature verification, subscription-event handling),
  and `/billing/{checkout,webhook,usage}`. Quota enforcement lives inside
  `require_api_key()` itself -- the one chokepoint every gated route
  already calls -- gated on `BILLING_ENABLED` so a self-hosted deployment
  never imports `app.billing` on its hot path at all.
- **Real bug caught during implementation, not left for later**: the
  first quota-enforcement design metered *every* authenticated call,
  including calls into `/billing/usage` and `/billing/checkout`
  themselves -- meaning checking your own usage could itself count
  against your quota, or even get blocked by it. Fixed by scoping
  metering to a `_BILLABLE_BLUEPRINTS` allowlist (`wallet_ownership`,
  `ageverify` only) inside `enforce_quota_and_record()`.
- **`DeveloperProject`** gained `plan_status`, `stripe_customer_id`,
  `stripe_subscription_id`, `plan_updated_at` -- `plan_status` is a cache
  of the last processed Stripe webhook, never a live API call, since
  `require_api_key()` is a hot path.
- **Migration reality check, twice.** Generating and applying the new
  Alembic migration against live Postgres surfaced two real, fixable
  problems that a Postgres-only test run would never have caught:
  1. Adding `developer_project_id NOT NULL` failed immediately against
     existing local dev rows in the wallet-ownership/age-verify tables
     (`NotNullViolation`) -- fixed by clearing those specific tables in
     the migration itself (accepted, since this is pre-production local
     data, not real customer data) rather than inventing a placeholder
     tenant.
  2. The same NOT NULL problem hit `developer_projects.plan_status`
     against *real* existing rows (test signups from earlier in this
     session) -- fixed with `server_default='free'` instead of deleting
     data that was worth keeping.
  3. Autogenerated `create_foreign_key(None, ...)` / unique-constraint
     calls work fine against Postgres (which accepts unnamed
     constraints) but hard-fail under SQLite's batch-mode table rebuild,
     which requires a real constraint name -- caught by a new
     `test_migrations.py` that actually runs `flask db upgrade` against
     a throwaway sqlite file, something the rest of the suite (which
     bypasses Alembic via `db.create_all()`) would never exercise. Fixed
     by naming every constraint explicitly in both `upgrade()` and
     `downgrade()`.
  4. That same test, run in-process the first time via
     `flask_migrate.upgrade()`, corrupted the pytest process's global
     logging configuration (Alembic's `env.py` calls
     `logging.config.fileConfig(...)`, which reconfigures the *root*
     logger for the whole process) and broke an unrelated, previously-
     passing `caplog`-based webhook test purely through file-execution
     order. Fixed by running the migration as a real subprocess instead
     (`python -m flask db upgrade`), which also better matches how the
     real Docker entrypoint invokes it.
- **Public deployment overlay**: `docker-compose.hosted.yml` (additive,
  base `docker-compose.yml` untouched) adds a Caddy reverse proxy with
  automatic HTTPS for a real domain. Documented explicitly, not silently
  left as a gap: Compose overlays merge `ports:` by concatenation, so the
  overlay cannot retract the base file's direct `8300:8300` publish --
  the hosted environment's firewall must expose only 80/443 publicly.
- **Pricing page** (`/developer/pricing`) branches on `BILLING_ENABLED` so
  a self-hosted instance never shows upgrade copy to its own operator.
- 24 new tests across `test_tenant_isolation.py`, `test_billing_metering.py`,
  `test_billing_quota_enforcement.py`, `test_billing_route.py`,
  `test_migrations.py`, plus pricing-page coverage -- full suite: 126/126
  passing.
